...
| # | Task | Pliki | Status |
|---|------|-------|--------|
| T1 | Approval state redesign (3 stany + comments passing) | `src/preview/server.js` (approval route) | DONE |
| T2 | MCP tool redesign `mockup_await_approval` | `src/mcp/tools/approval-tools.js` (NEW lub refactor) | DONE |
| T3 | Versioning integration (auto set status) | `src/mcp/tools/approval-tools.js` | DONE |
| T4 | Editor UI: 3 przyciski (Accept / Accept+comments / Reject) | `src/preview/editor/editor.js`, `approval.js` (NEW) | DONE |
| T5 | Comment migration przy nowej wersji | `src/storage/project-store.js` | DONE |
| T6 | MCP resource update `mockup://approval` | `src/mcp/resources.js` | DONE |
| T7 | Testy | `tests/mcp/approval-tools.test.js`, `tests/preview/approval-api.test.js` | DONE |
